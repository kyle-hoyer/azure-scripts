...
try
{
    ...
    $account = Add-AzureAccount -Environment AzureUSGovernment -Credential $azureCredential
	
    # Check for returned userID, indicating successful authentication
    if(Get-AzureAccount -Name $account.id)
    {
    ...
    if($subscriptions.Count -eq 1)
    {
        ...
        $resourceManagerContext = Add-AzureRmAccount -EnvironmentName AzureUSGovernment -Credential $azureCredential -SubscriptionId $targetSubscription.SubscriptionId 
